...
- (void)layoutSubviews
{
    ...
    self.titleLabel.frame = CGRectMake(EdgeMarin, EdgeMarin, SCREEN_WIDTH - 2*EdgeMarin, TitleHeight);
    self.horlelabel.frame=CGRectMake(SCREEN_WIDTH-EdgeMarin*3, EdgeMarin, SCREEN_WIDTH - 2*EdgeMarin, TitleHeight);
    ...
    //重新计算textView frame
    self.textView.frame = CGRectMake(EdgeMarin, CGRectGetMaxY(self.titleLabel.frame) + EdgeMarin, SCREEN_WIDTH - 2*EdgeMarin, MAX(DefaultTextViewHeight - 3*EdgeMarin - TitleHeight, newHeight - TitleHeight - 3*EdgeMarin));
}
...
+ (CGFloat)cellHeightWithItem:(SelwynFormItem *)item{
    ...
    CGSize detailSize = [SelwynFormHandle getSizeWithString:item.formDetail Font:[UIFont systemFontOfSize:TitleFont] maxSize:CGSizeMake(SCREEN_WIDTH - 4*EdgeMarin, MAXFLOAT)];
