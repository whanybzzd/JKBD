...
- (void)layoutSubviews
{
    ...
    self.horlelabel.frame = CGRectMake(SCREEN_WIDTH-EdgeMarin*5, EdgeMarin, TitleWidth, TitleHeight);
    CGFloat newHeight = [SelwynFormSelectTableViewCell cellHeightWithItem:_formItem];
    self.textView.userInteractionEnabled = NO;
    //重新计算textView frame
    self.textView.frame = CGRectMake(TitleWidth + 2*EdgeMarin, EdgeMarin, SCREEN_WIDTH - (TitleWidth + 2*EdgeMarin + 30),  MAX(TitleHeight, newHeight - 2*EdgeMarin));
}

/** 动态获取cell高度 */
+ (CGFloat)cellHeightWithItem:(SelwynFormItem *)item{
    
    CGSize detailSize = [SelwynFormHandle getSizeWithString:item.formDetail Font:[UIFont systemFontOfSize:TitleFont] maxSize:CGSizeMake(SCREEN_WIDTH - TitleWidth - 2*EdgeMarin - 30, MAXFLOAT)];
