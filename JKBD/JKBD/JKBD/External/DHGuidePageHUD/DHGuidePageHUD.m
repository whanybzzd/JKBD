...
- (instancetype)dh_initWithFrame:(CGRect)frame imageNameArray:(NSArray<NSString *> *)imageNameArray buttonIsHidden:(BOOL)isHidden {
    if ([super initWithFrame:frame]) {
        ...
        self.colorArray=@[@"#eb4040",@"#a759f1",@"#4760fb",@"#f52735"];
        // 设置引导视图的scrollview
        ...
        for (int i=0; i<imageNameArray.count; i++) {
            ...
            if (i == imageNameArray.count-1) {
                ...
                UIButton *startButton = [[UIButton alloc]initWithFrame:CGRectMake(DDScreenW*0.2, DDScreenH*0.9, DDScreenW*0.6, 35)];
                [startButton setTitle:@"立即体验" forState:UIControlStateNormal];
                [startButton setTitleColor:[UIColor colorWithHexString:@"#f52735"] forState:UIControlStateNormal];
                startButton.layer.cornerRadius=17.5;
                startButton.layer.borderColor=[UIColor colorWithHexString:@"#f52735"].CGColor;
...
//        self.imagePageControl.currentPageIndicatorTintColor = [UIColor colorWithHexString:@"#eb4040"];
//        [self addSubview:self.imagePageControl];
        
        
        self.pageControl=[[PageControl alloc] initWithFrame:CGRectMake(DDScreenW*0.0, DDScreenH*0.9, DDScreenW*1.0, DDScreenH*0.1)];
        self.pageControl.currentPage = 0;
        self.pageControl.numberOfPages=imageNameArray.count;
        self.pageControl.pageIndicatorTintColor = [UIColor lightGrayColor];
        self.pageControl.currentPageIndicatorTintColor = [UIColor colorWithHexString:@"#eb4040"];
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollview {
    ...
    self.pageControl.currentPageIndicatorTintColor = [UIColor colorWithHexString:self.colorArray[page]];
